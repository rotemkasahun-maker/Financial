...
export class GmailStateRepository {
  constructor({blobStore,encryptionKey,freezeController=new WriteFreezeController()}){this.blobStore=blobStore;this.encryptionKey=encryptionKey;this.freezeController=freezeController}
  async read(){const bytes=await this.blobStore.read();return bytes?decryptJson(bytes,this.encryptionKey):emptyState()}
  async write(state){this.freezeController.assertWritable();await this.blobStore.write(encryptJson(state,this.encryptionKey))}
  async update(mutator){this.freezeController.assertWritable();const state=await this.read(),result=await mutator(state);await this.write(state);return result}
}
...
export function createStateRepository(config, freezeController){const blobStore=config.stateBucket?new GcsBlobStore({bucket:config.stateBucket,object:config.stateObject}):new LocalFileBlobStore(config.stateFile);return new GmailStateRepository({blobStore,encryptionKey:config.stateEncryptionKey,freezeController})}
