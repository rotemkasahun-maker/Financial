...
export class ImportPipeline {
  ...
  async prepare(envelope, file) {
    ...
    const normalized={...extracted, merchant:extracted.merchant.trim(), total:Number(extracted.total), sourceMetadata:{householdId:envelope.householdId,userId:envelope.userId,deviceId:envelope.deviceId,sourceType:envelope.sourceType,sourceAccount:envelope.sourceAccount,externalSourceId:envelope.externalSourceId,importedAt:envelope.importedAt}};
