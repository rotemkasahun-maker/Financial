...
export function calculateSummary(transactions) {
  ...
  const governmentBenefits=transactions.filter(t=>t.financialType==='government_benefit').reduce((s,t)=>s+t.amount,0);
  const bankCredits=transactions.filter(t=>t.financialType==='bank_credit').reduce((s,t)=>s+t.amount,0);
  ...
  const otherIncomingFunds=familySupport+governmentBenefits+bankCredits+gifts;
