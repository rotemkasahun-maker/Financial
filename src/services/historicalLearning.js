...
export function analyzeHistoricalRecords(records=[],options={}){
  ...
  return {proposals:proposals.sort((a,b)=>({high:0,medium:1,conflict:2,low:3}[a.confidence]-{high:0,medium:1,conflict:2,low:3}[b.confidence])),ignored,summary:{total:proposals.length,high:proposals.filter(p=>p.confidence==='high').length,autoApprovable:proposals.filter(p=>p.confidence==='high'&&p.explicitUserEvidence>=2).length,medium:proposals.filter(p=>p.confidence==='medium').length,conflicts:proposals.filter(p=>p.confidence==='conflict').length,low:proposals.filter(p=>p.confidence==='low').length,recordsAnalyzed:records.length},readOnly:true,affectsTotals:false,createsXP:false,sessionId:options.sessionId||generateId('bootstrap')};
}
...
export const isSafeHistoricalAutoApproval=proposal=>proposal.confidence==='high'&&proposal.explicitUserEvidence>=2&&proposal.status!=='rejected';
export const bulkApproveSafeHistoricalRules=result=>result.proposals.filter(isSafeHistoricalAutoApproval).map(approveHistoricalProposal);
