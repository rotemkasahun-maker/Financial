...
export function normalizeRows(rows,{filename='import',selectedSource=null,classificationRules=[]}={}){
  ...
  for(let index=header.index+1;index<rows.length;index++){
    ...
    const savedRule=valid?applySavedClassificationRules(base,classificationRules):null,allocation=valid&&!savedRule?classifyCapitalAllocation(base):null,bankFee=valid&&!savedRule?classifyBankFee(base):null,rule=valid&&!savedRule&&!allocation&&!bankFee?classifyWithRules(base):null,financialType=savedRule?.financialType||bankFee?.financialType||allocation?.financialType||(direction==='debit'?'expense':'unknown'),category=savedRule?.category||bankFee?.category||allocation?.category||rule?.category||null,reviewReason=savedRule?.requiresReview?'התאמה לכלל שמור דורשת אישור':valid?deriveReviewReason({...base,financialType,category,allocationType:allocation?.allocationType}):null;
    normalized.push({...base,financialType,allocationType:allocation?.allocationType||null,category,classificationOrigin:savedRule?.origin||bankFee?.origin||allocation?.origin||rule?.origin||null,classificationRuleId:savedRule?.ruleId||null,classificationConfidence:savedRule?.confidence||null,classificationExplanation:savedRule?.explanation||null,reviewStatus:reviewReason?'required':'not_required',reviewReason,reviewDecision:null,importStatus:valid?'ready':'malformed',warnings:valid?[]:[parseFailureReason]});
  }
...
export function applyReviewDecision(row,decision){const mapping={expense:{financialType:'expense',category:row.category||'כללי'},income:{financialType:'income',category:'הכנסה שוטפת'},family_support:{financialType:'family_support',category:categoryFor('family_support')},gift:{financialType:'gift',category:categoryFor('gift')},refund:{financialType:'refund',category:categoryFor('refund')},reimbursement:{financialType:'reimbursement',category:'החזרים'},transfer:{financialType:'transfer',category:'העברה פנימית'},savings_transfer:{financialType:'savings_transfer',allocationType:'savings',category:'חיסכון'},investment_transfer:{financialType:'investment_transfer',allocationType:'investment',category:'השקעות'}};if(decision==='ignore')return {...row,excluded:true,reviewDecision:decision,reviewStatus:'resolved'};if(!mapping[decision])return row;return {...row,...mapping[decision],excluded:false,reviewDecision:decision,reviewStatus:'resolved',reviewReason:null}}
...
export async function createValidatedImportPreview(file,{parseXlsx,existingTransactions=[],existingReceipts=[],classificationRules=globalThis.__familyFinanceClassificationRules||[],logger=console.info}={}){
  const selected=await readSelectedImportFile(file,{parseXlsx,logger}),parsed=normalizeRows(selected.rows,{filename:selected.filename,classificationRules});diagnostic(logger,{stage:'normalized',filename:selected.filename,parserRowCount:selected.rows.length,normalizedRowCount:parsed.rows.length,validRowCount:parsed.rows.filter(row=>row.valid).length,errorCode:parsed.error?'header_not_detected':null});
