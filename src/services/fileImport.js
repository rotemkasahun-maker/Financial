...
export function normalizePostingStatus(value,{guaranteedPostingStatus=null}={}){const raw=String(value??'').trim();if(!raw)return guaranteedPostingStatus==='posted'?'posted':'unknown';const normalized=raw.toLowerCase();if(/pending|processing|authori[sz]ed|ממתין|בהמתנה|טרם|עתיד|בתהליך|זמני/u.test(normalized))return 'pending';if(/posted|booked|completed|final|בוצע|נרשם|סופי|הושלם/u.test(normalized))return 'posted';return 'unknown'}
...
export function normalizeRows(rows,{filename='import',selectedSource=null,classificationRules=[],reconciliationEvidence={},guaranteedPostingStatus=null}={}){
