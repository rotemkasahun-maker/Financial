...
export function buildImportPreview(parsed,{existingTransactions=[],existingReceipts=[]}={}){
  ...
  return {...parsed,rows,summary:{totalRows:rows.length,totalDebits:debits,totalCredits:credits,newTransactions:rows.filter(row=>row.importStatus==='ready').length,existingTransactions:rows.filter(row=>row.importStatus==='existing').length,possibleDuplicates:rows.filter(row=>row.duplicateStatus==='possible').length,requiresReview:rows.filter(row=>row.financialType==='unknown'&&row.valid).length,malformed:rows.filter(row=>!row.valid).length},dateRange:{from:included.map(row=>row.date).sort()[0]||null,to:included.map(row=>row.date).sort().at(-1)||null}};
}
