...
export function normalizeRows(rows,{filename='import',selectedSource=null,classificationRules=[],reconciliationEvidence={}}={}){
  ...
  for(let index=header.index+1;index<rows.length;index++){
    ...
    const savedRule=valid?applySavedClassificationRules(base,classificationRules):null,reconciled=valid&&!savedRule?reconcileBeforeReview(base,reconciliationEvidence):null,allocation=valid&&!savedRule&&!reconciled?classifyCapitalAllocation(base):null,bankFee=valid&&!savedRule&&!reconciled?classifyBankFee(base):null,knownIncoming=valid&&!savedRule&&!reconciled&&!allocation&&!bankFee?classifyKnownIncoming(base):null,rule=valid&&!savedRule&&!reconciled&&!allocation&&!bankFee&&!knownIncoming?classifyWithRules(base):null,financialType=savedRule?.financialType||reconciled?.financialType||bankFee?.financialType||allocation?.financialType||knownIncoming?.financialType||(direction==='debit'?'expense':'unknown'),category=savedRule?.category||reconciled?.category||bankFee?.category||allocation?.category||knownIncoming?.category||rule?.category||null,reviewReason=savedRule?.requiresReview?'התאמה לכלל שמור דורשת אישור':reconciled?.requiresReview?'נמצאה התאמה אפשרית במקור נוסף':valid?deriveReviewReason({...base,financialType,category,allocationType:allocation?.allocationType}):null;
    normalized.push({...base,financialType,allocationType:allocation?.allocationType||null,category,classificationOrigin:savedRule?.origin||(reconciled?'reconciliation':bankFee?.origin||allocation?.origin||rule?.origin||null),classificationRuleId:savedRule?.ruleId||null,classificationConfidence:savedRule?.confidence||reconciled?.confidence||null,classificationExplanation:savedRule?.explanation||reconciled?.explanation||null,reconciliationMatchId:reconciled?.matchingRecordId||null,reviewStatus:reviewReason?'required':'not_required',reviewSeverity:'non_critical',blocksImport:false,reviewReason,reviewDecision:null,importStatus:valid?'ready':'malformed',warnings:valid?[]:[parseFailureReason]});
  }
