...
export function classifyKnownIncoming(record){if(!['credit','incoming'].includes(record.direction))return null;const text=`${record.merchant||''} ${record.description||''}`.trim(),bankSource=record.sourceType==='bank_import';if(/משכורת|שכר עבודה|תשלום שכר/i.test(text))return {financialType:'income',category:'הכנסה שוטפת',origin:DecisionOrigin.HOUSEHOLD_RULE,confidence:.98};if(bankSource&&/(?:ביטוח לאומי|קצב(?:ת|אות)).*(?:ילד|ילדים)|(?:ילד|ילדים).*(?:ביטוח לאומי|קצבה)/i.test(text))return {financialType:'government_benefit',category:'קצבאות ומענקים',origin:DecisionOrigin.HOUSEHOLD_RULE,confidence:.99};if(bankSource&&/קבלת תשלום על יתרת זכות|ריבית.*יתרת זכות|זיכוי ריבית.*עו[״"]?ש/i.test(text))return {financialType:'bank_credit',category:'ריבית וזיכויים מהבנק',origin:DecisionOrigin.HOUSEHOLD_RULE,confidence:.99};return null}
