...
| Gmail A | User A | Thin backend implemented; cloud setup pending | Separate OAuth and relevance-filtered incremental sync | Automatic | Pub/Sub push; daily watch renewal | Metadata candidate → relevant body/link/attachment identity → receipt-evidence staging | Delivery/message/attachment ID + URL fingerprint | Invalid cursor → bounded catch-up; revoked OAuth → reconnect; uncertain evidence → existing review | Backend ready; credentials not connected |
| Gmail B | User B | Same connector/storage boundary | Same, isolated OAuth grant | Automatic | Same | Same as Gmail A | Same as Gmail A | Same as Gmail A | Backend ready; credentials not connected |
